...
```bash
uvicorn api.app:app --reload          # local backend (mongomock + infra/sample_kb)
```

| Endpoint | Purpose |
|---|---|
| `GET /health` | liveness + doc count + backend |
| `POST /search` | catalog search — titles/metadata for everyone |
| `POST /read` | read one doc — **content gated by the user's role** (fail-closed) |
...
Set `MONGODB_URI` (Amazon DocumentDB) and `KB_BACKEND=s3` + `KB_S3_BUCKET` to switch from
the local mongomock + files backend to DocumentDB + S3 — no code changes.

## Project layout

```
brain_agents/   the Admin Duty Agent: config, context, tools, guardrails, runtime
brain_kb/       knowledge base: Mongo catalog + access-gated S3 content + ingest
api/            FastAPI app (search / read / ingest / ask)
merchant/       buyer-side ordering: MerchantAdapter interface + mock + meal planner
infra/          Terraform (S3 + IAM) and a local sample KB / access simulator
vault/          Obsidian knowledge base (generated by seed_vault.py)
tests/          merchant, vault, spend-approval, KB-access, KB-service, and API tests
...
- ✅ KB retrieval: Mongo catalog + access-gated reads (Case-B multi-file) + FastAPI
- ⏳ Wire the KB search/read tools into the agent for `/ask`
- ⏳ DocumentDB + S3 backend (flip `MONGODB_URI` / `KB_BACKEND=s3`) · vector/semantic search

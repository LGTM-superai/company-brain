...
class KBService:
    ...
    def read(self, user: User, doc_id: str) -> ReadResult:
        ...
        # App-level access pre-check. In production with per-role IAM keys, S3 also
        # enforces this (defense in depth); with a single shared cred it's the gate.
        if not can_read(user.role, meta.s3_key_md):
            return ReadResult(

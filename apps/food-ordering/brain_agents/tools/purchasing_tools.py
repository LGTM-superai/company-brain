...
def place_order(
    ...
    # Real payment: charge the Stripe virtual card if configured (Stripe enforces the
    # spending limit). Otherwise fall back to the mock card-limit check.
    import os
    from merchant.stripe_issuing import StripeIssuing, configured

    stripe_note = ""
    if configured():
        si = StripeIssuing()
        res = si.authorize(os.environ["STRIPE_CARD_ID"], cart.total_cents, merchant_name=merchant_id)
        if not res["approved"]:
            return (f"DECLINED by Stripe: order {format_money(cart.total_cents)} exceeds the "
                    f"virtual card limit ({res.get('decline_reason') or 'spending control'}). "
                    f"Escalate to {ctx.context.payment.authorized_by} for approval.")
        si.capture(res["id"])
        stripe_note = f" Charged Stripe virtual card (auth {res['id']})."
    ...
    return (f"Order {order.id} placed with {merchant_id} for "
            ...
            + (stripe_note or f" Charged card {order.payment_card_id}."))

...
// ── Fast variants (simple searchAndContents, no agent run) ─────────────────
...
// ── Deep variants (Exa agent runs with structured output) ──────────────────
...
export async function queryExaSearch(query: string) {
  const result = await queryExaSearchDeep(query);
  if (!result.ok && result.error?.includes("timed out")) {
    return queryExaSearchFast(query);
  }
  return result;
}

async function queryExaSearchDeep(query: string) {
  try {
    const exa = new Exa(requireEnv("EXA_API_KEY"));
...
export async function queryExaCVE(query: string) {
  const result = await queryExaCVEDeep(query);
  if (!result.ok && result.error?.includes("timed out")) {
    return queryExaCVEFast(query);
  }
  return result;
}

async function queryExaCVEDeep(query: string) {
  try {
    const exa = new Exa(requireEnv("EXA_API_KEY"));
    const run = (await withTimeout(
      exa.beta.agent.runs.create({
...
export async function queryExaNews(query: string) {
  const result = await queryExaNewsDeep(query);
  if (!result.ok && result.error?.includes("timed out")) {
    return queryExaNewsFast(query);
  }
  return result;
}

async function queryExaNewsDeep(query: string) {
  try {
    const exa = new Exa(requireEnv("EXA_API_KEY"));
    const run = (await withTimeout(
      exa.beta.agent.runs.create({
